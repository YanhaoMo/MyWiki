# 我的 vim 配置
我应该是从大一听说vim这个东西之后，就一直在使用vim，一直以来，vim都蛮能满足我编程&写文档的需求。
我在长期使用vim的过程中积累了许多配置，包括各种能极大地扩展vim编辑功能的各种插件。在此记录一下，方便以后查阅。

首先是vim的插件管理器，我使用的是Vundle，Vundle能帮助vim用户方便地安装管理所有的vim插件。
不过其实我一直期待vim能有一个想emacs那样的内置的强大的插件管理器，但是目前的vim好像还没有实现这个。
...
* 静态语法检查 syntastic
* 多光标编辑 这个功能看起来比较新奇，但是感觉并没有什么比较常见的应用场景，或许可以用来处理重构，但是也处理的不好 vim-multi-cursor
* 代码重构支持
* 自动编译支持
* 调试支持
* 工程支持
